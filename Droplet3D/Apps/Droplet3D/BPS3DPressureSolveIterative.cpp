...
{
    template<class M>
    bool solveBiCGStab(M & mat, const VecXd & rhs, VecXd & x, int & iters, double tol_error = 1e-10, int maxiters = 1000)
    {
...
void BPS3D::step_PressureSolveIterative(double dt, VecXd & v, const Partitioning & partitioning)
{
...
//            assert(triple_junction_edges.size() == 2);
...
#ifdef DENSE_SOLVER
    std::vector<MatXd> dense_A(p2v.size());
    std::vector<VecXd> dense_rhs(p2v.size());
    for (size_t pi = 0; pi < p2v.size(); pi++)
    {
        size_t Npi = p2v[pi].size();
        dense_A[pi] = MatXd::Zero(Npi, Npi);
        dense_rhs[pi] = VecXd::Zero(Npi);
    }
    ...
    std::vector<std::vector<VecXd> > dense_rhs_per_vertex(p2v.size());
    for (size_t pi = 0; pi < p2v.size(); pi++)
    {
        size_t Npi = p2v[pi].size();
        dense_rhs_per_vertex[pi].assign(Npi, VecXd::Zero(Npi));
    }
...
//                        BoundaryIntegral::duffyTransform(xis, 0, qik, y, jacobian_i, c_i);
    ...
    for (size_t pi = 0; pi < p2v.size(); pi++)
    {
        ...
        VecXd dense_x = dense_rhs[pi];
        int dense_iters = 0;
        CSim::TimerMan::timer("Sim.step/BPS.step/explicit/BEM/solve/dense_solve").start();
        solveBiCGStab(dense_A[pi], dense_rhs[pi], dense_x, dense_iters, 1e-15);
        CSim::TimerMan::timer("Sim.step/BPS.step/explicit/BEM/solve/dense_solve").stop();
        solution[pi] = dense_x;
        std::cout << "BiCGStab iterations = " << dense_iters << std::endl;
#endif
        
#ifdef FMM_SOLVER
        CSim::TimerMan::timer("Sim.step/BPS.step/explicit/BEM/solve/fmm_setup").start();
        // create the mesh for this partition to feed into FMMPlan
        std::vector<Vec3d> pvs;     // vertices in this partition
        std::vector<FMMPlan::Vec3st> pfs;    // faces in this partition
        ...
        {
            size_t i = p2f[pi][pfi];
            LosTopos::Vec3st f = mesh().m_tris[i];
            pfs.push_back(FMMPlan::Vec3st(v2pi[f[0]], v2pi[f[1]], v2pi[f[2]]));
        }
        ...
        VecXd bc_p    = VecXd::Zero(p2f[pi].size() * 3);
        VecXd bc_dpdn = VecXd::Zero(p2f[pi].size() * 3);
        for (size_t pfi = 0; pfi < p2f[pi].size(); pfi++)
        {
            FMMPlan::Vec3st f = pfs[pfi];
        ...
        CSim::TimerMan::timer("Sim.step/BPS.step/explicit/BEM/solve/fmm_setup").stop();

        CSim::TimerMan::timer("Sim.step/BPS.step/explicit/BEM/solve/fmm_rhs").start();
        VecXd fmm_rhs = -fmm_A.evaluateGreensRepresentation(bc_p, bc_dpdn);
        CSim::TimerMan::timer("Sim.step/BPS.step/explicit/BEM/solve/fmm_rhs").stop();
        ...
        VecXd fmm_x = fmm_rhs;
        int fmm_iters = 0;
        CSim::TimerMan::timer("Sim.step/BPS.step/explicit/BEM/solve/fmm_solve").start();
        solveBiCGStab(fmm_A, fmm_rhs, fmm_x, fmm_iters, 1e-15);
        CSim::TimerMan::timer("Sim.step/BPS.step/explicit/BEM/solve/fmm_solve").stop();
        solution[pi] = fmm_x;
